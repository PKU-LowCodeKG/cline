...
/**
 * Task 类是 Controller 类的核心成员属性。【原 Cline 类和 ClineProvider 类】
 *
 * 对于用户的一个新任务或者历史任务，在 Controller 中创建一个 Task 实例处理，一个 Task 实例只负责处理一个任务，由 taskId 标识。
 *
 * Task 实例的生命周期是：开始新任务/恢复并继续旧任务（创建 Task 实例）-> 中断废弃当前任务（废弃 Task 实例）
 */
export class Task {
	// dependencies
	...
	/** 当前 Task 实例要处理的任务 ID，是 Date 字符串 */
	...
	/**
	 * 表示“当前 Task 实例绑定的任务”是否被废弃。它只在 abortTask() 中被改为 true。
	 *
	 * 所有 `abortTask()` 的地方，紧跟着 `this.task = undefined`，即 Task 实例被废弃。
	 * 说明 Task 实例的生命周期完全和其绑定的任务一致。
	 */
	private abort: boolean = false
	didFinishAbortingStream = false
	/**
	 * 表示“ Task 实例是否被废弃”。它只在 Controller 的 cancelTask() 中被改为 true。
	 * 说明 Task 实例的生命周期完全和其绑定的任务一致。
	 */
	abandoned = false
	...
	/**
	 * 更新当前 Task 实例绑定的任务 的 时间戳、API 消耗指标等
	 *
	 ...
	 * @param offset - 可选的偏移量，用于调整 clineMessages 消息索引【因为会截断？】
	 */
	 ...
	 * 由 `handleWebviewAskResponse()` 函数设置 Task 实例的 askResponse 3 个属性。
	 *
	 * 【ask 和 say 的共性】
	...
	async ask(
		...
		// NOTE: 这段话是说：【Cline 类现在改为了 Task 类】
	...
	/**
	 * 如果当前 Task 实例的 clineMessages 历史队列的最后一条消息 “不完整”，且其 ask 或 say 属性与提供的 askOrSay 参数相同，则：
	 * 1. 删除该消息。
	...
	// #region Task 实例绑定任务的生命周期：开始新任务/恢复并继续旧任务、中断当前任务
	/**
	 * 【主线】基于任务描述 task 或者 提供的图片 image 开始一个新任务
	 * 1. 清空 Task 实例的 对话历史（API）和 ClineMessage 消息（webview 显示）
	 * 2. say("text", task, images)
	 * 3. 初始化任务循环
	 * 【这个函数只在 Task 构造函数使用，再次说明 Task 实例的生命周期 和 任务 相同，一个 Task 实例只能处理一个 任务】
	 */
	...
	/**
	 * 【主线】基于 Task 实例构造时的 historyItem 属性，继续完成一个旧任务【恢复一个历史任务】
	...
	private async resumeTaskFromHistory() {
		...
		// NOTE: 从 ClineMessage 数组中找到最后一个非 “恢复历史任务” 的消息（因为之前可能进行过多次“恢复任务”）
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.abort) {
			// NOTE: 只在（while 的第一次循环，下面这个函数的第一层递归）在 <environment_detail> 中包括当前工作目录下的文件列表。
	...
	/**
	 * 【主线】中断（废弃）Task 实例绑定的当前任务。
	 *
	 * 该函数是唯一修改 this.abort 的地方。因此 说是“中断”，实际上是彻底废弃了当前任务。
	 * 此外，所有 `abortTask()` 的地方，紧跟着 `this.task = undefined`，即 Task 实例被废弃。
	 * 因此，Task 实例会在 废弃其绑定的任务后 被垃圾回收。
	 *
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		// NOTE: Cline 更改了获取 vscode.ExtensionContext 的方式，使用了 getContext() 函数
		logMessages(ollamaMessages, this.getContext().globalStorageUri.fsPath)
	...
	async presentAssistantMessage() {
		...
		switch (block.type) {
			...
			case "tool_use":
				 ...
				 *    添加到 Task 实例的 userMessageContent 属性
	...
	async recursivelyMakeClineRequests(userContent: UserContent, includeFileDetails: boolean = false): Promise<boolean> {
		...
		try {
			...
			try {
				...
				// NOTE: abandoned 和 abort 的区别是，abandoned 是当扩展不再等待 cline 实例【现 Task 实例】完成中止时发生的（当循环中的任何函数由于 this.abort 而抛出错误时，此处会抛出错误）
	...
	async loadContext(userContent: UserContent, includeFileDetails: boolean = false): Promise<[UserContent, string, boolean]> {
		...
		const processUserContent = async () => {
			...
			// 简单来说，因为在别的地方（具体函数见英文注释）会把 1.用户拒绝工具调用并提供反馈 或者 2.用户回答 Cline 提出的 followup question 的内容 放在标签内，这样就会有四种标签 表示用户输入了内容。
				...
				userContent.map(async (block) => {
					if (block.type === "text") {
						...
						if (
							...
							// NOTE: 这里修改了 parseMentions 的参数，增加了 Task 实例【原 Cline 实例】
							...
							// Cline 更新：支持 以斜杠 / 开头的命令，如 /newtask "xxx"
							// 当 解析用户输入的以斜杠 / 开头的命令时，我们仍然希望允许用户提供他们想要的上下文（即保留命令后面的文本内容作为语义输入）
